...
namespace GSCommerceAPI.Services.SUNAT
{
    public class FacturacionElectronicaService : IFacturacionElectronicaService
    {
        ...
        private string GenerarXmlFactura(ComprobanteCabeceraDTO dto)
        {
            ...
            sb.AppendLine($"<cbc:Name>{EscaparTextoXml(dto.RazonSocialEmisor)}</cbc:Name>");
            sb.AppendLine("</cac:PartyName>");
            sb.AppendLine("<cac:PartyLegalEntity>");
            sb.AppendLine($"<cbc:RegistrationName>{EscaparTextoXml(dto.RazonSocialEmisor)}</cbc:RegistrationName>");
            ...
            sb.AppendLine($"<cbc:CityName>{EscaparTextoXml(dto.ProvinciaEmisor)}</cbc:CityName>");
            sb.AppendLine($"<cbc:CountrySubentity>{EscaparTextoXml(dto.DepartamentoEmisor)}</cbc:CountrySubentity>");
            sb.AppendLine($"<cbc:District>{EscaparTextoXml(dto.DistritoEmisor)}</cbc:District>");
            sb.AppendLine("<cac:AddressLine>");
            sb.AppendLine($"<cbc:Line>{EscaparTextoXml(dto.DireccionEmisor)}</cbc:Line>");
            sb.AppendLine("</cac:AddressLine>");
            ...
            sb.AppendLine("</cac:PartyIdentification>");
            sb.AppendLine("<cac:PartyLegalEntity>");
            sb.AppendLine($"<cbc:RegistrationName>{EscaparTextoXml(dto.NombreCliente)}</cbc:RegistrationName>");
            sb.AppendLine("</cac:PartyLegalEntity>");
            sb.AppendLine("</cac:Party>");
            sb.AppendLine("</cac:AccountingCustomerParty>");
            // Para facturas y boletas se envía la forma de pago. En notas de crédito/debito no se requiere
